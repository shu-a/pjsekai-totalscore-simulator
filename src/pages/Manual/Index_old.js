...
export default function ManualIndex() {
  ...
  const content = card.map(c => {
    i = i + 1;
    return <CardInfo key={i} alt={c.alt} imgSrc={c.imgSrc} title={c.title} content={c.content} imgContent={c.imgContent} />
  });
  return (
    <Box display="flex" justifyContent="center" alignItems="center" sx={{ flexWrap: 'wrap' }}>
      {content}
      <CardManual />
    </Box>
  );
