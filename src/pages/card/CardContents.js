...
export default function CardContents(props) {
  ...
  useEffect(() => {
    ...
    if (team && loadTeam === "N") {
      const subUnitEvent = {
        target: { name: subUnitName, value: formValue[subUnitName] },
      };
    ...
    if (loadSubUnit === "Y") {
      if (team && loadTeam === "N") {
        const characterEvent = {
          target: { name: characterName, value: formValue[characterName] },
        };
  ...
  const teamList = props.teamList.map((c) => (
    <MenuItem key={c.seq} value={c.unit} unit={c.unit}>
      {c.unitName}
    </MenuItem>
  ));
  const attrList = props.attrList.map((c) => (
    <MenuItem key={c.unit} value={c.unit}>
      {c.unitName}
    </MenuItem>
  ));
  const raritiesList = props.raritiesList.map((c) => (
    <MenuItem key={c.seq} value={c.seq}>
      {c.cardRarityType.split("_")[1]}
    </MenuItem>
  ));
  ...
  const makeFormSelectContents = [
    ...
    },
    {
      ...
      id: "subUnit",
      label: "서브유닛",
      selectSx: { color: readOnlySubUnit ? "rgba(0, 0, 0, 0.38)" : "" },
      value: subUnit,
      handler: handleSelectSubUnit,
      selectList: teamList,
      helperText: helperText,
      readonly: readOnlySubUnit,
    },
    {
      sx: { m: 1, width: 120 },
      id: "rarities",
      label: "별",
      value: rarities,
      handler: handleSelectRarities,
      selectList: raritiesList,
    },
    {
      sx: { m: 1, width: 120 },
      id: "character",
      ...
      selectSx: { color: readOnlySubUnit ? "rgba(0, 0, 0, 0.38)" : "" },
      value: character,
      handler: handleSelectCharacter,
      selectList: charactList,
      helperText: helperText,
      readonly: readOnlyCharacter,
    },
  ];
